...
func _render() -> void:
	...
	if _page == "report":
		...
		_label(PlayerCopy.SAFETY_SCREEN_25F225BDB8EB)
	...
	if _page == "block":
		...
		_label(PlayerCopy.SAFETY_SCREEN_2FEE2DCBAAB6)
		_button("Block player", _block)
		_button("Cancel", func(): _page = "home"; _render())
		return
	if _page == "clear_report":
		_label(PlayerCopy.SAFETY_SCREEN_9922536928E7, 25)
		_label(PlayerCopy.SAFETY_SCREEN_EA0ECD988D13)
	...
	if _owner_context.is_empty():
		...
		if client.terms_accepted: _label(PlayerCopy.SAFETY_SCREEN_BC57296AFA63)
		else:
			_label(PlayerCopy.SAFETY_SCREEN_14484BCCA0FD)
	...
	if not _owner_context.is_empty() and not terms_only:
		...
		if not pending.is_empty():
			_label(PlayerCopy.SAFETY_SCREEN_D83D834525F5)
		...
		if not blocked.is_empty():
			...
			_label(PlayerCopy.SAFETY_SCREEN_7C2E596D74AB)
...
func _wait_api() -> bool:
	...
	while _current() and is_instance_valid(client._api) and client._api.busy:
		if Time.get_ticks_msec() >= deadline: _message = PlayerCopy.SAFETY_SCREEN_45CAEF70E0A7; return false
...
func _accept() -> void:
	...
	if not _current(): return
	_busy = false
	_message = PlayerCopy.SAFETY_SCREEN_98CB7DC115E5 if ok else client.last_error
	_render()
...
func _report(reason: String) -> void:
	...
	_busy = false
	_page = "home"
	_message = PlayerCopy.SAFETY_SCREEN_258F2B933FD3 if ok else client.last_error
	_render()
...
func _report_request(retry: bool) -> void:
	...
	if not _current(): return
	_busy = false
	_message = PlayerCopy.SAFETY_SCREEN_EB1718025BA7 if ok else client.last_error
	_render()

func _clear_report() -> void:
	_message = PlayerCopy.SAFETY_SCREEN_084C3C974A7B if client.clear_pending_report() else client.last_error
	_page = "home"
	_render()
...
func _unblock(player: String) -> void:
	...
	if not _current(): return
	_busy = false
	_message = PlayerCopy.SAFETY_SCREEN_FEB4A4F8B768 if ok else client.last_error
	_render()

func _open_link(path: String) -> void:
	if not client.open_public(path): _message = PlayerCopy.SAFETY_SCREEN_99C248C9E5D3; _render()
