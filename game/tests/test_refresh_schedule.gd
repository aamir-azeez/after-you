...
func _test_room_and_lobby() -> void:
	...
	_check(clock.next_due_ms() == 16000, "An active room schedules fifteen seconds after binding")
	clock.bind("identity-1/epoch-1/room-a", 15000)
	_check(clock.next_due_ms() == 16000, "Repeated context observation does not postpone polling")
	_check(clock.begin_if_due(15999, true, false).is_empty(), "A room does not poll before its due time")
	var token: Dictionary = clock.begin_if_due(16000, true, false)
	_check(not token.is_empty() and clock.busy(), "A due eligible room reserves one request")
	_check(clock.complete(token, 16500, true), "The matching completion is accepted")
	_check(not clock.busy() and clock.next_due_ms() == 31500, "Successful polling schedules from completion, without catch-up bursts")
	clock.bind("identity-1/epoch-1/lobby", 20000, Schedule.LOBBY_INTERVAL_MS)
	_check(clock.next_due_ms() == 50000, "A lobby uses the thirty-second cadence")
	clock.bind("", 21000)
...
func _test_coalescing_and_guards() -> void:
	...
	_check(clock.next_due_ms() == 115001, "No burst is accumulated during a long request")
...
func _test_context_change() -> void:
	...
	_check(clock.next_due_ms() == 15021, "An old identity failure does not apply its backoff to the new identity")
...
func _test_backoff_and_cooldown() -> void:
	...
	_check(clock.next_due_ms() == 30100, "First failed read backs off thirty seconds")
	token = clock.begin_if_due(30100, true, false)
	clock.complete(token, 30200, false)
	_check(clock.next_due_ms() == 90200, "Second failed read backs off sixty seconds")
	token = clock.begin_if_due(90200, true, false)
	clock.complete(token, 90300, false)
	_check(clock.next_due_ms() == 210300, "Third failed read backs off one hundred twenty seconds")
	token = clock.begin_if_due(210300, true, false)
	clock.complete(token, 210400, false)
	_check(clock.next_due_ms() == 330400, "Repeated failures stay at the bounded delay")
	...
	_check(clock.next_due_ms() == 225500, "Successful recovery restores the ordinary cadence")
	token = clock.begin_if_due(225500, true, false)
	clock.complete(token, 225600, false)
	_check(clock.next_due_ms() == 255600, "A new failure after success restarts at the first backoff")
