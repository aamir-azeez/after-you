...
func _run() -> void:
	...
	for name: String in ["a-little-lift-a", "a-little-lift-b", "a-place-to-grow-a", "a-place-to-grow-b"]:
		...
		if name == "a-little-lift-b":
			_check(app.mode == "checkpoint" and app.journey.stage_id() == "a-place-to-grow", "A midpoint checkpoint needs explicit Continue")
			var reopened := _journey(path)
			reopened.load_data()
			_check(not reopened.read_only and reopened.stage_id() == "a-place-to-grow", "Checkpoint is actually durable on reopening")
			app._show_ready()
	...
	print("AFTER YOU FIRST STEPS PREVIEW: %d checks, %d failures" % [checks, failures])
...
func _handed_controls() -> void:
	for left_handed: bool in [false, true]:
		...
		root.add_child(viewport)
		var app := _preview()
		app.journey = _journey(_new_path("handed-controls"))
...
func _resume_and_storage_failures(app: Node3D) -> void:
	...
	app.journey = _journey(path, storage)
	...
	app._begin()
	var first := _fixture("a-little-lift-a")
	...
	storage.failures_remaining = 1
	_press(app, "Save this contribution")
	...
	app._begin()
	var receiver_inputs := Simulation.expand_recording_inputs(_fixture("a-little-lift-b"))
...
func _fixture(name: String) -> Dictionary:
	var json := JSON.new()
	if json.parse(FileAccess.get_file_as_string("res://tests/fixtures/first_steps/" + name + ".json")) != OK:
...
func _preview() -> Node3D:
	var app := Preview.new()
	app.chapter_key = Registry.FIRST_STEPS
	return app

func _journey(path: String, storage: RefCounted = null) -> RefCounted:
	return Journey.new(path, storage, Registry.FIRST_STEPS)
