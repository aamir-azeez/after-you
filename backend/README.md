...
Presence is optional, foreground-only state, separate from room revisions, recordings, photos and notifications. `PRESENCE_ENABLED` defaults to `false`. With it enabled, an authenticated `POST /v1/presence` accepts exactly `{schema_version:1,session_id,online}`. The session ID is 18 random bytes encoded as 36 lowercase hexadecimal characters; `online` is a Boolean. Both online and offline acknowledgements are `{schema_version:1,heartbeat_seconds:30,expires_after_seconds:90}`: these describe the policy, not the remaining lease. Clients publish at most every 30 seconds and use a new session ID for each foreground publication interval. Offline removes only that session, so it cannot hide another active session. Requests within one session use server arrival order; clients must serialize them.

At most eight current-credential leases are retained per identity. A ninth new live session returns `409 presence_session_limit`; refreshing an existing session remains allowed. Expired leases are pruned on reads/writes and by a Player-owned expiry alarm. Credential recovery and identity deletion synchronously invalidate all leases. No historical last-seen value is exposed or retained as history.

`GET /v1/rooms/:room_id/presence` and `GET /v2/rooms/:room_id/presence` return exactly `{schema_version:1,partner_joined,partner_online,expires_after_seconds}`. The latter field is remaining lifetime, an integer from 0 to 90; offline or an unpaired room returns 0. Only current authenticated room members can read the paired player's presence, including when they do not publish their own. A block in either direction or a deleted room prevents the read. There is no public player-ID presence lookup. Responses are not cached. When the feature is disabled, online heartbeats and room reads return `503 presence_unavailable`; authenticated offline cleanup remains available. Clients should display unavailable reads as unknown rather than offline.

Player presence tables and their exact owned alarm are ephemeral service state. They are validated but omitted from portable snapshots; restored identities remain offline until a fresh heartbeat. Existing gameplay archive formats and Durable Object namespaces are unchanged. A presence-aware Worker is required to handle cleanup alarms and export databases containing these tables; feature disable uses that compatible code rather than an older strict-schema exporter. Existing player request limits also apply to presence.
