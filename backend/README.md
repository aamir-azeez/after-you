...
The existing portable snapshot implementation covers **Player and v1 Room only**. Player format 2 preserves v2 links, but does not back up the separate RoomV2 namespace. RoomV2 has schema metadata and fixed tables but deliberately exposes no portable export/restore method yet. Add and validate a versioned RoomV2 snapshot implementation, inventory and deletion/restore coordination before representing it as an owner-held backup. Do not roll back to a release that lacks v2-aware room-link listing/deletion after v2 data exists.
