...
This source adds `/v2` room coordination for the exact bundled **Relay Isles**, level/simulation/recording version 2, with two consecutive A/B pairs. It is not a migration of old rooms or a claim that a native online-v2 client has shipped. `V2_ROOMS_ENABLED` is `"false"` by default. Enabling it requires a separate reviewed deployment decision; local tests override this variable in their isolated environment. Disabling it later holds creation, joins, turns and forks while preserving authenticated reads, accepted-operation lookup and deletion. Recovery continues through the existing `/v1/identity/recover` protocol and retains both versions of room membership.
...
`test/v2.test.ts` covers native fixture hashes and nested checkpoints, role changes, exact retry receipts after advancement/eviction, source mismatches, malformed/deep data, forks, membership, shared identity recovery/deletion and old-room isolation. Local runtime validation must pass before enabling the feature; an API fixture commit is not proof of native online gameplay. No media upload or public image route is included.

RoomV2 provides binding-only `exportSnapshot(sourceCommit)` and `restoreSnapshot(archive, expectedLogicalId)` methods. These use archive format 3/database schema 2, independently of Player/v1 Room formats. The public HTTP router exposes neither method. Exports preserve exact stored JSON, positive 64-bit row IDs, both active and historical pairs, and immutable operation receipts. Metadata must match the pinned schema; unexpected SQL objects, KV values or alarms cause a refusal. A canonical SHA-256 envelope detects accidental alteration; it is not authentication or encryption, so use trusted, privately stored archives.
