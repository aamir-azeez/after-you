...
function installed(storage: DurableObjectStorage): boolean { return [4, 5].includes(storage.sql.exec<{ schema_version: number }>("SELECT schema_version FROM metadata WHERE id=1").one().schema_version); }
