...
export const PHOTO_DELIVERY_TABLE: TableDefinition = {
  name: "photo_delivery", schema: "CREATE TABLE photo_delivery (turn_id TEXT PRIMARY KEY, data TEXT NOT NULL)", columns: ["rowid", "turn_id", "data"], maxRows: 128,
  select: "SELECT CAST(rowid AS TEXT) AS rowid,turn_id,data FROM photo_delivery ORDER BY photo_delivery.rowid LIMIT 129", insert: "INSERT INTO photo_delivery (rowid,turn_id,data) VALUES (CAST(? AS INTEGER),?,?)"
};
export const ROOM_V2_DELIVERY_TABLES: readonly TableDefinition[] = [...ROOM_V2_REACTION_TABLES, PHOTO_DELIVERY_TABLE];
