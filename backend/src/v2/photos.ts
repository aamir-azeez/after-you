...
function read(storage: DurableObjectStorage, turnId: string): PhotoState | null {
...
function metadata(state: PhotoState | null): PhotoMetadata | null { if (!state) return null; const { jpeg_base64: _bytes, ...safe } = state; return safe; }
...
export function getPhoto(storage: DurableObjectStorage, room: Room | null, player: string, turnId: string): Outcome<{ photo: PhotoMetadata | null; jpeg_base64: string | null }> {
  ...
  const state = read(storage, turnId);
  return ok({ photo: metadata(state), jpeg_base64: state?.jpeg_base64 ?? null });
}
...
export function getPhotoOperation(storage: DurableObjectStorage, room: Room | null, player: string, key: string): Outcome<PhotoMutation> {
  ...
  return ok({ receipt, photo: metadata(read(storage, receipt.turn_id)) });
}
...
export function mutatePhoto(storage: DurableObjectStorage, room: Room | null, player: string, input: ParsedPhotoMutation): Outcome<PhotoMutation> {
  ...
  const previous = read(storage, input.turn_id);
  ...
  return ok({ receipt, photo: metadata(state) });
}
