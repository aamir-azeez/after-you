...
import { LEGACY_ROOM_V2_TABLES, METADATA_SCHEMA, ROOM_V2_TABLES, ROOM_V2_REACTION_TABLES, initializePairReactions, initializePhotoDelivery, ROOM_V2_DELIVERY_TABLES } from "./storage-schema";
...
export type RoomV2Archive = { payload: {
  format: "after-you-object-snapshot"; format_version: 3 | 4 | 5 | 6 | 7; database_schema_version: 2 | 3 | 4 | 5; object_kind: "RoomV2";
...
function definitions(version: number) { return version === 2 ? LEGACY_ROOM_V2_TABLES : version === 3 ? ROOM_V2_TABLES : version === 4 ? ROOM_V2_REACTION_TABLES : ROOM_V2_DELIVERY_TABLES; }
function schema(storage: DurableObjectStorage): 3 | 4 | 5 {
  const metadata = storage.sql.exec<{ id: number; schema_version: number }>("SELECT id,schema_version FROM metadata LIMIT 2").toArray();
  need(metadata.length === 1 && metadata[0].id === 1 && (metadata[0].schema_version === 3 || metadata[0].schema_version === 4 || metadata[0].schema_version === 5), "unsupported_storage_schema");
  const version = metadata[0].schema_version as 3 | 4 | 5;
...
export async function validateRoomV2(serialized: string, expectedLogicalId: string | null): Promise<RoomV2Archive> {
  ...
  need(p.format === "after-you-object-snapshot" && (legacy || ((p.format_version === 4 || p.format_version === 5) && p.database_schema_version === 3) || (p.format_version === 6 && p.database_schema_version === 4) || (p.format_version === 7 && p.database_schema_version === 5)) && p.object_kind === "RoomV2", "unsupported_snapshot_format");
  ...
  need(!checked.newChapter || p.format_version === 5 || p.format_version === 6 || p.format_version === 7, "unsupported_snapshot_format");
  ...
  return { payload: { format: "after-you-object-snapshot", format_version: p.format_version as 3 | 4 | 5 | 6 | 7, database_schema_version: p.database_schema_version as 2 | 3 | 4 | 5, object_kind: "RoomV2", logical_id: checked.logicalId, source_object_id: p.source_object_id, source_commit: p.source_commit, exported_at: String(p.exported_at), summary: checked.summary, tables: copied }, checksum: { algorithm: "SHA-256", value: checksum.value } };
}
