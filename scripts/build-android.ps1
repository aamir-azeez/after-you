...
try {
    ...
    $keyPath = Join-Path $signing ('after-you-' + $Configuration.ToLowerInvariant() + '.keystore')
    $passwordPath = Join-Path $signing ('after-you-' + $Configuration.ToLowerInvariant() + '.password.dpapi')
    $alias = 'after-you-' + $Configuration.ToLowerInvariant()
    ...
    if (!(Test-Path -LiteralPath $keyPath)) {
        ...
        & "$JdkPath/bin/keytool.exe" -genkeypair -keystore $keyPath -storetype PKCS12 -alias $alias -keyalg RSA -keysize 4096 -validity 10000 -dname 'CN=After You, O=Ampiere Labs' -storepass:env AFTERYOU_KEY_PASSWORD -keypass:env AFTERYOU_KEY_PASSWORD 2>&1 | ForEach-Object { Write-Output ($_.ToString().Replace($password, '[redacted]')) }
